...
# refresh time for the perpetual motion
    ...
    Button(buttons, text='Start',bg='yellow',command=game.start).grid(pady=10,padx=12,ipadx=14,ipady=4)
    Button(buttons, text='Restart',bg='blue',command=game.clean).grid(pady=10,padx=12,ipadx=9,ipady=4)
    Button(buttons, text='Quit',bg='red', command=root.destroy).grid(pady=10,padx=12,ipadx=15,ipady=4)
    buttons.grid(column=0, row=0)
    scoreboard = Frame(root, width=35, height=2*HT/5)
    Label(scoreboard, text='Game Score',font=('franklin gothic book',18)).grid()
   
    Label(scoreboard, textvariable=game.score.counter,font=('franklin gothic book',20)).grid()
    Label(scoreboard, text='High Score',font=('franklin gothic book',18)).grid()
    Label(scoreboard, textvariable=game.score.maximum,font=('franklin gothic book',20)).grid()
